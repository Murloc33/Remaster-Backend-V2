    ...
    return JSONResponse(
        content={
            "data": AthleticsPlaceData(
                ...
                disciplines_with_minimum_number_of_participants_participation=[10, 11, 12]
            ).model_dump()
        }
    )
            ...
            "data": AdditionalConditions(
                additional_conditions=[
                    AdditionalCondition(
                        ...
                        min_participants= value["win_match"] if value["win_match"] != 0 else None,
                    ) for value in data
                ]
            ).model_dump()
        }
...
def check_result(
    ...
    if discipline_id in (10, 11, 12) and not second_condition:
