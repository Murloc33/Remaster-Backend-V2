...
from modules.athletes.modules.athlete.schemes import UpdateAthlete
...
def update_athlete(
    ...
    cursor.execute(
        (
            ...
            "full_name = ?, birth_date = ?, sport_id = ?, municipality_id = ?,"
            "organization_id = ?, is_sports_category_granted = ?, is_doping_check_passed = ?"
            "WHERE id = ?"
        ),
        (
            athlete.full_name, athlete.birth_date, athlete.sport_id,
            athlete.municipality_id, athlete.organization_id, athlete.is_sports_category_granted,
